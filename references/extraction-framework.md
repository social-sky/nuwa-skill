...
## 五B、多人物综合视角（Multi-Teacher Synthesis）

当合并多个视角时，使用以下策略：

### 共识提取
- 多个人物对同一问题看法一致 → 高置信度结论
- 引用时标注「A 和 B 都認為...」
...
### 互补组合
- A 的盲區由 B 補上
- 如：Jobs 的產品品味 + Musk 的成本拆解 = 完整產品策略

### 路由选择
根据问题类型分配最适人物：
- 产品/设计问题 → 优先 Jobs 或 张小龙
- 成本/工程问题 → 优先 Musk
- 风险/投资问题 → 优先 塔勒布 或 芒格
- 教学/简化问题 → 优先 费曼
...
## 六、质量自检清单（heuristic + 量化）

生成Skill后，用以下问题自检。分为 heuristic 检查项（通過/不通過）和量化評分項（0-100）。
...
### 量化评分参考（与 SKILL.md Phase 4 一致）

| 维度 | 目标分数 | 最低通过 | 测量方式 |
|------|---------|---------|---------|
| 已知立场正确性 | ≥85 | ≥70 | 5个已知问题 vs Skill回答 |
| 表达DNA符合度 | ≥80 | ≥60 | 3个开放问题 x 5子维度 |
| 框架生成力 | ≥80 | ≥60 | 2个未公開問題的推斷品質 |
| 对抗性区分 | — | <65%判別器正確率 | 5段真人 vs 5段Skill 混合盲測 |
| 內在一致性 | ≥85 | ≥70 | 3輪追問下的立場穩定度 |
| 誠實性 | ≥90 | ≥80 | 資訊不足時是否誠實承認 |
